...
public class CodingDNASequence extends DNASequence
{
   ...
   @return See {@link #checkStartCodon()}.   
	...
	{
		if(this.seqLength() >= 3 && 
		this.toString().substring(0,3).toUpperCase().equals("ATG")) {
			return true;
		}
		return false;
	}
   ...
   @throws RuntimeException See {@link #translate()}.
	...
	public char[] translate()
	{
		...
		String seqarr = this.toString();
		String protein = "";
		String codon = "";
		char aminoAcid;
		
		for(int i = 0; i < seqarr.length() - 4; i += 3) {
			codon = (seqarr.substring(i, Math.min(i+3, seqarr.length()-1)));
			
			aminoAcid = getAminoAcid(codon);
			if(aminoAcid == '$')
				break;
